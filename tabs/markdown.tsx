...
export default function MarkdownPage() {
    ...
    function poll() {
      chrome.storage.local.get(["pageData"], (result) => {
        ...
        if (Date.now() - startTime < MAX_POLL_MS) {
          ...
          setError(
            "No page data found. The content script may not have finished extracting.\n\nTry triggering the extension again, or paste content manually."
          )
          setLoading(false)
        }
      })
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-950 text-zinc-100 overflow-hidden font-sans">
      ...
      {loading && !error && (
        <div className="flex-1 flex items-center justify-center gap-3 text-zinc-400 text-sm">
          <svg
